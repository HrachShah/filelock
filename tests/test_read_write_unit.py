...
def test_timeout_for_sqlite_rejects_non_numeric_timeout(timeout: object) -> None:
    with pytest.raises(TypeError, match="timeout must be a number"):
        timeout_for_sqlite(timeout, blocking=True, already_waited=0.0)


@pytest.mark.parametrize("timeout", [float("nan"), float("inf"), float("-inf")])
def test_timeout_for_sqlite_rejects_non_finite_timeout(timeout: float) -> None:
...
@pytest.mark.parametrize("timeout", [3_000_000.0, 1e307])
